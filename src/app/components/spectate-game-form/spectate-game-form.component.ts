...
@Component({
  selector: 'app-spectate-game-form',
  templateUrl: './spectate-game-form.component.html',
  styleUrls: ['./spectate-game-form.component.scss'],
  standalone: true,
  imports: [
    FormsModule,
    ReactiveFormsModule,
...
export class JoinGameFormComponent {
  ...
  submit() {
    if (this.form.valid) {
      ...
      this.restGameService.get(gameFriendlyId).subscribe({
        next: (gameDto) => {
